...
"use strict";
angular.module("edup.common", ["restangular", "angularUtils.directives.dirPagination"]), angular.module("edup.common").directive("appDatePicker", function () {
    return {
        restrict: "A", scope: {selectedDate: "=pickerValue"}, link: function (a, b) {
            b.datetimepicker(), b.on("dp.change dp.hide", function (b) {
                a.selectedDate = b.date
            })
        }
    }
}), angular.module("edup.common").directive("appModal", function () {
    return {
    ...
    a.setPath("/vendor/bower_components/angular-utils-pagination/dirPagination.tpl.html")
}]), angular.module("edup.common").run(["Restangular", "UrlService", function (a, b) {
    a.setBaseUrl(b.BaseUrl), a.setErrorInterceptor(function (a) {
        return console.log(angular.toJson(a, !0)), !1
    }), a.setRestangularFields({etag: "versionId"})
}]), angular.module("edup.common").service("PaginationService", function () {
    var a = function (a) {
        return a.page * a.perPage
    }, b = function (a) {
        return a.page * a.perPage - a.perPage
    };
    return {Top: a, Skip: b}
}), angular.module("edup.common").service("RestService", ["Restangular", function (a) {
    var b = a.one("private");
    return {Students: b.one("students"), Balance: b.one("balance")}
...
}), angular.module("edup.students", []), angular.module("edup.students").controller("StudentAttendanceController", ["$scope", function (a) {
...
}]), angular.module("edup.students").controller("StudentDocumentsController", ["$scope", function (a) {
    a.documents = [{name: "report.docx", date: "2015/03/03", link: "https:/localhost:8443/edup/api/file/report.docx"}, {
        name: "balance.xls",
        date: "2015/03/04",
        link: "https:/localhost:8443/edup/api/file/balance.xls"
    }, {name: "photo.jpg", date: "2015/03/04", link: "https:/localhost:8443/edup/api/file/photo.jpg"}]
}]), angular.module("edup.students").controller("StudentIdentificationCardController", ["$scope", function () {
}]), angular.module("edup.students").controller("StudentsController", ["$scope", "$timeout", "RestService", "PaginationService", function (a, b, c, d) {
    function e(a) {
        return !a || 0 === a.length
    }

    a.studentSelected = !1, a.basicSearch = {spin: !1}, a.paging = {enabled: !1, page: 1, perPage: 10, totalRecords: 0};
    var f = function (a, b, c, d) {
        var e = {};
        return e.$count = !0, a && (e.$top = a), a && (e.$skip = b), a && (e.$search = c), d && (e.$orderby = d), e
    };
    a.loadFullStudent = function (b) {
        b && (a.basicSearch.spin = !0, c.Students.one(b.toString()).get().then(function (b) {
            a.selectedStudent = b.payload, a.selectedStudent.balance = b.payload.balance / 100, a.studentSelected = !0, a.basicSearch.spin = !1
        }))
    }, a.loadStudents = function (b, d, e, g) {
        a.basicSearch.spin = !0;
        var h = f(d, e, g, "Created desc");
        c.Students.get(h).then(function (c) {
            a.students = c.values, a.paging.totalRecords = c.count, a.students.length > 0 ? a.loadFullStudent(b ? b : a.students[0].id) : a.studentSelected = !1, a.basicSearch.spin = !1
        })
    }, a.loadStudents(null, d.Top(a.paging), d.Skip(a.paging)), a.setSelected = function (b) {
        a.loadFullStudent(b)
    }, a.addToBalance = function (a) {
        console.log("Add new balance " + a)
    }, a.pageChanged = function (b, c) {
        a.paging.page = b, a.loadStudents(null, d.Top(a.paging), d.Skip(a.paging), c)
    ...
    a.executeSearch = function (c) {
        c && c.length > 2 ? b(function () {
            a.paging.page = 1, a.loadStudents(null, d.Top(a.paging), d.Skip(a.paging), c), g = c
        }, 300) : e(c) && g !== c && b(function () {
            a.paging.page = 1, a.loadStudents(null, d.Top(a.paging), d.Skip(a.paging), null), g = c
        }, 300)
    }, a.executeStudentUpdate = function (b) {
        b && b.name && b.lastName && b.id && b.versionId && c.Students.one(b.id.toString()).customPUT(b).then(function (c) {
            a.newStudent = null, a.photoUrl = null, a.photoUploaded = !1, b.versionId = c.payload, a.loadStudents(b.id, d.Top(a.paging), d.Skip(a.paging), null)
        })
    }
}]), angular.module("edup.students").directive("studentAttendance", function () {
    return {
        restrict: "E", templateUrl: "student-attendance", link: function () {
        }
    }
}), angular.module("edup.students").directive("studentBalance", function () {
    return {
        restrict: "E", templateUrl: "student-balance", link: function (a) {
            a.newBalanceValue = "", a.openDatePicker = function () {
    ...
    return {
        restrict: "E", templateUrl: "student-documents", link: function (a) {
            a.directiveTest = "Student tabbed pane directive"
        }
    }
...
}), angular.module("edup.students").directive("studentInformation", function () {
    return {
        restrict: "E", templateUrl: "student-information", link: function () {
        }
    }
}), angular.module("edup.students").directive("studentsList", function () {
    return {
        restrict: "E", templateUrl: "students-list", link: function () {
        }
    }
...
}), angular.module("edup.widgets", ["angularFileUpload", "fiestah.money"]), angular.module("edup.students").controller("NewStudentModalController", ["$scope", "RestService", "PaginationService", function (a, b, c) {
    a.executeStudentSave = function (d) {
        d && d.name && d.lastName && b.Students.customPOST(d).then(function (b) {
            a.dismissModal(), a.newStudent = null, a.photoUrl = null, a.photoUploaded = !1, a.loadStudents(b.payload, c.Top(a.paging), c.Skip(a.paging), null)
        })
    }, a.executeCancel = function () {
        a.newStudent = null
    }
}]), angular.module("edup.widgets").directive("balanceModal", ["RestService", function (a) {
    return {
        restrict: "E", templateUrl: "balance-modal", link: function (b) {
            b.price = {amount: 0}, b.saving = function (c) {
                if (c && c.amount) {
                    var d = {studentId: b.selectedStudent.id, amount: 100 * c.amount, comments: c.comment};
                    a.Balance.customPOST(d).then(function (a) {
                        var d = a.payload;
                        d && (b.selectedStudent.balance += c.amount, b.balance = null, b.dismissModal())
                    })
                }
...
}]), angular.module("edup.widgets").directive("fileUpload", ["UrlService", "FileUploader", "$window", function (a, b, c) {
    return {
        restrict: "E", templateUrl: "file-upload", scope: !1, priority: 10, link: function (d) {
            d.uploader = new b({url: a.BaseUrl + "/private/files/upload"}), d.uploader.removeAfterUpload = !0, d.deleteItem = function (a) {
                console.log("delete " + a)
            }, d.openDownloadUrl = function (a) {
                c.open(a, "_blank")
            }, d.uploader.onWhenAddingFileFailed = function (a, b, c) {
                console.info("onWhenAddingFileFailed", a, b, c)
            }, d.uploader.onAfterAddingFile = function (a) {
                console.info("onAfterAddingFile", a)
            }, d.uploader.onAfterAddingAll = function (a) {
                console.info("onAfterAddingAll", a)
            }, d.uploader.onBeforeUploadItem = function (a) {
                console.info("onBeforeUploadItem", a)
            }, d.uploader.onProgressItem = function (a, b) {
                console.info("onProgressItem", a, b)
            }, d.uploader.onProgressAll = function (a) {
                console.info("onProgressAll", a)
            }, d.uploader.onSuccessItem = function (a, b, c, d) {
                console.info("onSuccessItem", a, b, c, d)
            }, d.uploader.onErrorItem = function (a, b, c, d) {
                console.info("onErrorItem", a, b, c, d)
            }, d.uploader.onCancelItem = function (a, b, c, d) {
                console.info("onCancelItem", a, b, c, d)
            }, d.uploader.onCompleteItem = function (a, b, c, d) {
                console.info("onCompleteItem", a, b, c, d)
            }, d.uploader.onCompleteAll = function () {
                console.info("onCompleteAll")
            }
        }
    }
}]), angular.module("edup.widgets").directive("newStudentRecord", function () {
    return {
        restrict: "E", templateUrl: "new-student-modal", link: function () {
        }
    }
}), angular.module("edup.widgets").directive("photoUpload", ["UrlService", "FileUploader", "$window", function (a, b, c) {
    return {
        restrict: "E", templateUrl: "photo-upload", scope: {id: "=photoId", photoUpdate: "=", photoUrl: "="}, priority: 10, link: function (d) {
            d.photoUrl = null, d.photoUploaded = !1, d.uploader = new b({url: a.Files.Upload}), d.uploader.removeAfterUpload = !0, d.deleteItem = function (a) {
                console.log("delete " + a)
            }, d.openDownloadUrl = function (a) {
                c.open(a, "_blank")
            }, d.uploader.onSuccessItem = function (b, c) {
                d.id = c.payload.id, d.photoUrl = a.Files.Download + "/" + d.id, d.photoUploaded = !0
            }, d.uploader.onErrorItem = function (a, b, c, d) {
                console.info("onErrorItem", a, b, c, d)
            }
        }
    }
}]), angular.module("edup.tabs", ["edup.widgets", "edup.calendar", "edup.students"]), angular.module("edup", ["edup.tabs", "edup.login", "edup.header", "edup.footer", "edup.common"]), angular.module("edup").config(["$stateProvider", function (a) {
